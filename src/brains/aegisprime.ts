...
const aegisprime: Brain = {
  ...
  kind: g.kinds.bear,
  author: 'komme',
  description: 'Efficient threat control with predictive dodging, smart telekinesis, and precision fire. Made with Codex (gpt5.3)',
  ...
  thinkAboutIt(self, enemies, bullets, objects, events) {
    ...
    if (target && self.level >= 1 && self.energy >= g.telekinesisEnergyCost && tick - lastSpellTick >= 3) {
      ...
      if (edgeDist < 130 && dist < 420) {
        ...
        if (ex < edgeDist + 1) pushAngle = Math.PI; // left
        else if (ex > W() - edgeDist - 1) pushAngle = 0; // right
        else if (ey < edgeDist + 1) pushAngle = -Math.PI / 2; // up
        else pushAngle = Math.PI / 2; // down
