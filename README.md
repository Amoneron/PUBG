# Programmers Unknown BattleGround

**Programmers Unknown BattleGround** — визуальная арена, где алгоритмы (боты) управляют существами и сражаются в реальном времени. Каждый тик движка вызывает функцию `thinkAboutIt()` у каждого бота, передавая ему состояние мира. Бот анализирует обстановку и возвращает действие: двигаться, стрелять, прыгать, лечиться или использовать спецспособность.
...
На арене одновременно сражаются **только 4 существа** (настраивается параметром `maxAliveCreatures`, допустимый диапазон: 3-8). Все зарегистрированные боты (сейчас 13) **ротируются** по системе round-robin:

1. **При старте** на арену выходят первые 4 бота из перемешанного списка.
2. **При гибели** существо удаляется с арены, а его «мозг» помечается как мёртвый.
3. **Каждые 10 логических тиков** движок проверяет: если на арене меньше 4 существ, спавнится следующий мёртвый бот из очереди (round-robin по списку мозгов).
4. **Возрождение с нуля** — при респауне существо теряет всё: уровень сбрасывается на 0, здоровье = 100, энергия = 100, 1 патрон. Сохраняется только IQ (персистентный рейтинг) и счётчики kills/deaths.
5. **Уникальность** — один бот не может присутствовать на арене дважды одновременно. Бот ждёт своей очереди в ротации.
6. **Случайная позиция** — каждый респаун происходит в случайной точке арены с случайным углом взгляда.

Таким образом, бот встречается не со всеми 13 противниками сразу, а с 3 случайными в каждый момент времени. Это создаёт разнообразные тактические ситуации: иногда бот оказывается среди слабых врагов и может быстро набрать убийства и уровень, а иногда — среди сильных, и тогда выживание становится приоритетом.

### Звёзды

Звёзды — это особые объекты, которые появляются при **уничтожении препятствий**. Когда пуля разрушает препятствие, с вероятностью **30%** (`starsProbability = 0.3`) на его месте появляется звезда случайного типа.

Звёзды видны в массиве `objects` с `type = 2` (star). Тип звезды определяется полем `shape`:

| Shape | Название | Эффект | Вероятность |
|-------|----------|--------|-------------|
| 0 | **Level-up** | Мгновенное повышение уровня (+HP, +энергия, +ёмкость пуль) | Равная* |
| 1 | **Healing** | Полное восстановление HP и энергии до максимума текущего уровня | Равная* |
| 2 | **Poison** | Отравление: существо теряет по 1 HP каждый логический тик в течение 50 тиков (-50 HP суммарно) | Равная* |
| 3 | **Frozen** | Заморозка: существо не может выполнять действия 30 тиков | Равная* |
| 4 | **Death** | Взрыв: на арене появляются 4 динамита, все пули становятся резиновыми и разлетаются в случайных направлениях | ~10% |

\* Shapes 0-4 выбираются случайно (`randomInt(0, 4)`), но затем с вероятностью 10% результат заменяется на Death Star (shape 4). Итоговые вероятности: shapes 0-3 — по ~18% каждый, shape 4 (Death) — ~28%.

**Стратегические советы:**
- **Level-up** (shape 0) — самая ценная звезда, даёт мгновенный апгрейд уровня
- **Healing** (shape 1) — полезна при низком HP
- **Poison** (shape 2) и **Frozen** (shape 3) — опасны, их следует избегать
- **Death** (shape 4) — крайне опасна, создаёт хаос на арене
- Тип звезды можно определить по полю `shape` объекта и держаться подальше от вредных звёзд
...
### CC Opus — Splitpus (v7 Wide Stalker)
**Автор:** Claude | **Сила:** 10/10

Боевой сталкер с 11-фазным деревом решений. Расширенное обнаружение боёв (400px) для перехвата чужих драк и добивания раненых. Агрессивное преследование с низким порогом энергии (30%). Предиктивное прицеливание с упреждением по скорости цели. Score-based выбор целей: приоритет раненым и слабым врагам. Прыжковое уклонение на близкой дистанции (<70px). Без спецспособности — вся энергия идёт на боевые действия.
...
### _rat — Splitpus
**Автор:** _rat | **Сила:** 8/10

Боевой сталкер с предиктивным прицеливанием. Рассчитывает упреждение по скорости и вектору цели, отслеживает раненых врагов через события тика, подходит к чужим дракам для добивания. Оптимизирует выбор целей по IQ-награде. Цитирует Сунь-Цзы.

### Mamba — Runchip, Ядовитые пули
**Автор:** gramenbos | **Сила:** 7/10

Ядовитый охотник с механикой мести. Стреляет по динамиту рядом с врагами, использует двухтактовую систему turn+shoot для точных выстрелов, убегает от вооружённых врагов при отсутствии патронов. Отслеживает цель мести через события убийств.
...
Паразитический rhino. Следует за слабыми врагами как «хост», используя их как прикрытие. Предсказывает движение целей для точных выстрелов, взрывает динамит рядом с врагами, использует магнит для кражи пуль. Атакует застоявшихся хостов после 60 тиков неподвижности.
...
### AegisPrime — Bear, Телекинез
**Автор:** komme | **Сила:** 7/10

Контролёр с предиктивным уклонением. Рассчитывает ближайшую точку на траектории пули для точного доджа. Использует телекинез (2 энергии!) для отталкивания вооружённых угроз, вдавливания врагов в стены и притягивания жертв. Точная стрельба с допуском PI/40.
...
│   │   ├── opus.ts              # CC Opus — Splitpus, wide stalker
...
│   │   ├── troll.ts             # Troll — Rhino, магнит
│   │   ├── meowstraponius.ts    # Meowstraponius — Runchip, ядовитые пули
│   │   ├── mamba.ts             # Mamba — Runchip, ядовитые пули
│   │   ├── rat.ts               # _rat — Splitpus, fight-stalker
│   │   ├── aegisprime.ts        # AegisPrime — Bear, телекинез
...
Programmers Unknown BattleGround распространяется под лицензией [MIT](LICENSE.md).
