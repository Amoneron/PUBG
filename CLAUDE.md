...
## Ветки и правила коммитов

### master (origin/master)
Основная ветка. Сюда коммитятся ТОЛЬКО чистые изменения проекта:
- Новые/обновлённые боты
- Изменения движка
- Обновления README, конфигурации
- **НЕ коммитить** диагностические скрипты, логи, lab-файлы

### opus-lab (ТОЛЬКО ЛОКАЛЬНАЯ)
Ветка для итеративного улучшения бота CC Opus. **НИКОГДА не пушить в origin.**
Содержит:
- `src/headless-diag.ts` — диагностический headless с детальными логами убийств/смертей
- `scripts/run-tests.sh` — скрипт массового прогона тестов
- `data/diag/` — результаты диагностических прогонов
- Любые экспериментальные файлы

**Рабочий процесс:**
1. Разработка и тестирование бота ведётся в `opus-lab`
2. Когда бот готов — переключиться на `master`, скопировать только `src/brains/opus.ts`
3. Коммит и пуш в origin/master
4. Вернуться в `opus-lab`, merge master
